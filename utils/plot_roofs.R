...
fpeak_roofs <- function(df, types=NULL){
  fpeaks = data.frame(type=character(0), perf=numeric(0), sd=numeric(0), nthreads=integer(0), stringsAsFactors=FALSE)
  ...
  for(i in 1:length(types)){
    ...
    if(nrow(s) <= 0){next}
    ns = data.frame(type=types[i], perf=median(s$GFlop.s), sd=sd(s$GFlop.s), nthreads=s$n_threads[1], stringsAsFactors=F)
    fpeaks = rbind(fpeaks,ns)
  }
  fpeaks
}

bandwidth_roofs <- function(df, types=NULL){
  bandwidths = data.frame(obj=character(0), type=character(0), val=numeric(0), sd=numeric(0), nthreads=integer(0), stringsAsFactors=FALSE)
  ...
  for(type in types){
    ...
    if(nrow(s) <= 0){next}
    ns = data.frame(obj=as.character(type[1]), type=as.character(type[2]), val=median(s$GByte.s), sd=sd(s$GByte.s), nthreads=s$n_threads[1], stringsAsFactors=F)
    bandwidths = rbind(bandwidths, ns)
  }
  bandwidths
}

validation_points <- function(df, type, obj){
  ...
  samples = df[!is.infinite(df$Flops.Byte) & df$Flops.Byte>0 & df$type==type & df$memory==obj,]
  ois = unique(samples$Flops.Byte)
  for(oi in ois){
    s = samples[samples$Flops.Byte==oi,]
    if(nrow(s) <= 0){next}
    ns = data.frame(oi=oi, perf=median(s$GFlop.s), sd=sd(s$GFlop.s), info=obj, type=type, stringsAsFactors=FALSE)
    ret = rbind(ret,ns)
  }
  ret
}

data_points <- function(pt, types=NULL){
  ...
  for(type in types){
    ...
    ns = data.frame(oi=median(s$Flops/s$Bytes), 
                    perf=median(s$Flops/s$Nanoseconds), 
...
plot_bandwidths <- function(bandwidths, fpeak_max, xlim, ylim, col_start=1){
  ...
  for(i in 1:nrow(bandwidths)){
    val = bandwidths$val[i]
    perf = sapply(oi*val, min, fpeak_max)
    plot(oi, perf, lty=1, type="l", log="xy", axes=FALSE, xlab="", ylab="", xlim=xlim, ylim=ylim, col=col_start+i-1)
    par(new=T)
    if(options$stats){
      sd = bandwidths$sd[i]
...
plot_fpeaks <- function(fpeaks, xmin){
  for(i in 1:nrow(fpeaks)){
    abline(h = fpeaks$perf[i], lty=3, col=1, lwd=2);
    axis(2, labels = fpeaks$type[i], at=fpeaks$perf[i], las=1, tick=FALSE, pos=xmin, padj=0, hadj=0)
  }
  par(new=T)
}

plot_validation <-function(pts, col=1, pch=1, cex=.4){
  points(pts$oi, pts$perf, pch=pch, col=col, cex=cex)
  par(new=T)
  if(options$stats){
    segments(x0 = pts$oi, x1 = pts$oi, y0 = pts$perf-pts$sd*0.5, y1=pts$perf+pts$sd*0.5, col=col, lty=1)
  }
}
...
plot_data <-function(pts, xlim, ylim, col_start=1, pch_start=1, cex=1){
  ...
  for(i in 1:nrow(pts)){
    ...
    symbols(pts$oi[i], pts$perf[i], circles=1, inches=r, add=T, fg=colors[i], bg=adjustcolor(colors[i],alpha.f=.25), xlim=xlim, ylim=ylim, lwd=.5)
    points(pts$oi[i], pts$perf[i], pch=1, col=colors[i], cex=.3)
    if(options$stats){
      segments(x0 = pts$oi[i], x1 = pts$oi[i], y0 = pts$perf[i]-pts$sd[i]*0.5, y1=pts$perf[i]+pts$sd[i]*0.5, col=colors[i], lty=1)
    }
  }
...
roofline_plot <- function(df, bandwidths, fpeaks, validation=F, data=NULL){
  #Set limit and ticks
  fpeak_max = max(fpeaks$perf)
  ...
  if(validation){
    invisible(sapply(1:nrow(bandwidths), function(i){plot_validation(validation_points(df, bandwidths$type[i], bandwidths$obj[i]), col=i)}))
  }
  
  #plot layout
  axis(1, at=xticks, labels=xlabels)
  for (i in 1:nrow(fpeaks)){
    j=1; flops = as.numeric(fpeaks$perf[i])
  ...
  yticks = c(yticks, fpeaks$perf)
  ylabels = c(ylabels, sprintf("%.2f", as.numeric(fpeaks$perf)))
  axis(2, at=yticks, labels=ylabels, las=1)
  title(main = sprintf("%s (%s)", options$title, df$location[1]), xlab="Flops/Byte", ylab="GFlops/s")
  legend("bottomright",
         legend=sprintf("%s %s=%.2fGB/s", bandwidths$obj, bandwidths$type, bandwidths$val),

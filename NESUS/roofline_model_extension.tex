...
Usually, local memory bandwidth and is bound by the sequential accesses to it when only one thread is performing transactions
with it, whereas remote memories bandwidth are bound by the interconnection network bandwidth.
...
set it to the number of Core on the chip (which is not optimal) because applications are usually run on full machines.
